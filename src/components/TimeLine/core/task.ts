...
import { debug, unitWidth, halfUnitWidth, currentGroup, setCurrentGroup, barHeight, barMargin, arrowSize, showArrow, baseDate, dayMs } from './config'
...
export function getLeftHandleBar(w, box, taskBarRect, redrawChart, unitWidthOverride, halfUnitWidthOverride) {
  ...
  leftBar.on("dragend", function (e) {
    ...
    console.log(offsetX)
    const task = window.tasks[this.parent.index];
    task.start += offsetX;
...
export function getTaskBarMoveLine (chartStartX, chartStartY, lastScrollX, timeScaleWidth, posY, unitWidthOverride) {
  // 如果越界，不要画蓝色底线
  if (posY < 0 || posY > window.tasks.length - 1) return null
  const widthUnit = unitWidthOverride || unitWidth
