...
import { syncLocal, getRandomColor, getLocal, initData, updateData } from './data'
...
export function initGantt({
  ...
  const seedTasks = Array.isArray(initialTasks) && initialTasks.length
    ? [...initialTasks]
    : (useLocal ? getLocal() : defaultDemoTasks)
  // ensure trailing empty row for creating tasks
  if (!seedTasks.length || seedTasks[seedTasks.length - 1]?.name) {
    seedTasks.push({})
  }
  const tasks = seedTasks.map(normalizeTask)
  ...
  const mileStones = Array.isArray(initialMileStones)
    ? [...initialMileStones]
    : (useLocal ? getLocal('mileStones') : [
      { start: todayOffset + 10, name: t('gantt.demoMilestoneName') }
    ])
